...
function transpileImports(codeOriginal, filePath, isValueFile) {
    ...
    assertWarning(isPageConfigFile, `${filePathToShowToUser} is a JavaScript header file (.h.js), but JavaScript header files should only be used for +config.h.js, see https://vite-plugin-ssr.com/header-file`, { onlyOnce: true });
    // Remove the imports
    ...
    if (!isHeader) {
        ...
        assertWarning(false, `Rename ${filePathToShowToUser} to ${filePathCorrect}, see https://vite-plugin-ssr.com/header-file`, { onlyOnce: true });
    }
    return { code, fileImports };
}
...
function assertFileImports(fileImports, fileExports, filePath) {
    ...
    assertWarning(fileImportsUnused.length === 0, [
        ...
        `But the import${singular ? '' : 's'} ${importNamesUnused} ${singular ? "isn't" : "aren't"} re-exported at ${pc.cyan('export default { ... }')} and therefore ${singular ? 'has' : 'have'} no effect, see explanation at https://vite-plugin-ssr.com/header-file`
    ].join('\n'), { onlyOnce: true });
}
