...
function showWarnings(options) {
    assertWarning(false, '`createPageRenderer()` is outdated, use `renderPage()` instead. See https://vite-plugin-ssr.com/renderPage', { onlyOnce: true, showStackTrace: true });
    ...
    }
    {
        ...
        assertWarning(opts.length === 0, `The options ${str(opts)} you passed to \`createPageRenderer()\` have no effect. See https://vite-plugin-ssr.com/migration-0.4`, { onlyOnce: true });
    }
}
function str(opts) {
    return opts
